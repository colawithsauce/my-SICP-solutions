...
;;
;; NOTE: Resort to answers, thanks @jpath at http://community.schemewiki.org/?sicp-ex-2.43.
;;
;; Exchanging the order of the mapping in the flatmap results in queen-cols
;; being re-evaluated for every item in (enumerate-interval 1 board-size).
;; Therefore the whole work has to be duplicated board-size times at every
;; recursion level. Since there are always board-size recursions this means that
...
;;
;; So we can draw a priciple here: When cascading `map' and `flatmap', the time
;; consuming procedures should be positioned in the outer.
